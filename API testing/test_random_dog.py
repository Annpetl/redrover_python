...
def test_random_dog(dog_api):
    ...
    with allure.step('переобразование ответа из json в cловарь'):
        response = response.json()
        assert response['status'] == 'success', f'Ожидаемый статус:"success", актуальный {response['status']} '
...
@pytest.mark.parametrize("file", ['.md', '.MD', 'jpg', '.txt'])
